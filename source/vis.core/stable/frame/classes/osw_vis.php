...
class osW_VIS extends osW_Object {
	...
	public function getTools() {
		...
		$this->data['tools'][vOut('vis_login_module')]='Logon';
		$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis_tool: WHERE tool_status=:tool_status: ORDER BY tool_name ASC');
	...
	*/
	public function getToolsWithDetails() {
		...
		$this->data[$name][vOut('vis_login_module')]=array(
			'tool_name_intern'=>vOut('vis_login_module'),
			'tool_name'=>'Logon',
		);
		$QselectTools = osW_Database::getInstance()->query('SELECT tool_id, tool_name, tool_name_intern, tool_description, tool_status, tool_hide_logon, tool_hide_navigation FROM :table_vis_tool: WHERE tool_status=:tool_status: ORDER BY tool_name ASC');
		$QselectTools->bindTable(':table_vis_tool:', 'vis_tool');
		$QselectTools->bindInt(':tool_status:', 1);
		$QselectTools->execute();
		if ($QselectTools->query_handler===false) {
	...
	*/
	public function getToolDetails($tool_id=0) {
		...
		$QselectTool = osW_Database::getInstance()->query('SELECT * FROM :table_vis_tool: WHERE tool_id=:tool_id:');
		$QselectTool->bindTable(':table_vis_tool:', 'vis_tool');
	...
	*/
	public function getToolsSelectLogonArray() {
		...
		foreach ($this->getToolsWithDetails() as $tool_details) {
			if (($tool_details['tool_name_intern']!=vOut('vis_login_module'))&&($tool_details['tool_hide_logon']==0)) {
				$this->data[$name][]=array(
	...
	*/
	public function getToolsSelectArraybyPermission() {
		...
		$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis_tool: AS t INNER JOIN :table_vis_user_tool: AS u ON (u.tool_id=t.tool_id) WHERE t.tool_status=:tool_status: AND u.user_id=:user_id: ORDER BY t.tool_name ASC');
		$QselectTools->bindTable(':table_vis_tool:', 'vis_tool');
		$QselectTools->bindTable(':table_vis_user_tool:', 'vis_user_tool');
		$QselectTools->bindInt(':tool_status:', 1);
		$QselectTools->bindInt(':user_id:', osW_VIS_User::getInstance()->getId());
		$QselectTools->execute();
		if ($QselectTools->query_handler===false) {
			$this->__initDB();
			$QselectTools->execute();
	...
	*/
	public function getToolsSelectLogonArraybyPermission() {
		...
		$this->data[$name]=array();
		$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis_tool: AS t INNER JOIN :table_vis_user_tool: AS u ON (u.tool_id=t.tool_id) WHERE t.tool_status=:tool_status: AND u.user_id=:user_id: AND t.tool_hide_navigation=:tool_hide_navigation: ORDER BY t.tool_name ASC');
		$QselectTools->bindTable(':table_vis_tool:', 'vis_tool');
		$QselectTools->bindTable(':table_vis_user_tool:', 'vis_user_tool');
		$QselectTools->bindInt(':tool_status:', 1);
		$QselectTools->bindInt(':user_id:', osW_VIS_User::getInstance()->getId());
	...
	*/
	...
	public function setTool($tool) {
		...
		if ($this->validateTool($tool)===true) {
			$this->tool=$tool;
			osW_Session::getInstance()->set('vis_tool', $this->tool);
			return true;
		}
		return false;
	}
	...
	*/
	...
	public function checkToolPermissionByEMail($tool, $email) {
		if (osW_VIS_User::getInstance()->getIdByEMail($email)!==false) {
			return $this->checkToolPermissionbyID($tool, osW_VIS_User::getInstance()->getIdByEMail($email));
		}
		return false;
	}

	public function checkToolPermissionByID($tool, $user_id) {
		$QselectTool = osW_Database::getInstance()->query('SELECT * FROM :table_vis_tool: WHERE tool_name_intern=:tool_name_intern: AND tool_status=:tool_status:');
		$QselectTool->bindTable(':table_vis_tool:', 'vis_tool');
		...
		if ($QselectTool->numberOfRows()==1) {
			...
			$QloadData->bindInt(':tool_id:', osW_VIS::getInstance()->getToolIdByModule($tool));
			$QloadData->execute();
	...
	public function getUsernameById($user_id) {
		...
		if (!isset($this->data[$name])) {
			$this->data[$name]=array();
			$Quser = osW_Database::getInstance()->query('SELECT * FROM :table_vis_user: WHERE 1');
			$Quser->bindTable(':table_vis_user:', 'vis_user');
			$Quser->execute();
