...
class osW_VIS_Group extends osW_Object {
	...
	*/
	public function getListArray($key='group_id', $value='group_name') {
		...
		$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis_group: WHERE group_status=:group_status: AND tool_id=:tool_id: ORDER BY :order: ASC');
		$QselectTools->bindTable(':table_vis_group:', 'vis_group');
		$QselectTools->bindInt(':group_status:', 1);
		$QselectTools->bindInt(':tool_id:', osW_VIS::getInstance()->getToolID());
	...
	public function getGroupsByUserId($user_id=0) {
		...
		$QloadData->bindInt(':tool_id:', osW_VIS::getInstance()->getToolId());
		$QloadData->execute();
	...
	public function getGroupsNameInternByUserId($user_id=0) {
		...
		$QloadData = osW_Database::getInstance()->query('SELECT * FROM :table_vis_user_group: AS ug INNER JOIN :table_vis_group: AS g ON (g.group_id=ug.group_id) WHERE ug.user_id=:user_id: AND ug.tool_id=:tool_id:');
		$QloadData->bindTable(':table_vis_user_group:', 'vis_user_group');
