...
class osW_VIS_Navigation extends osW_Object {
	...
	public function addNavigationElement($data=array(), $tool_id=0) {
		...
		if (strlen($data['page_name_intern'])>0) {
			foreach ($data['permission'] as $flag) {
				osW_VIS_Permission::getInstance()->addPermissionByUserId($data['page_name_intern'], $flag, 0, $tool_id);
			}
		}
	...
	public function loadNavi($tool_id=0) {
		...
		$Qselect=osW_Database::getInstance()->query('SELECT *, n.tool_id, n.page_id FROM :table_vis_navigation: AS n LEFT JOIN :table_vis_page: AS p on (p.tool_id=n.tool_id AND p.page_id=n.page_id) WHERE n.tool_id=:tool_id: ORDER BY n.navigation_parent_id ASC, n.navigation_sortorder ASC, n.navigation_title ASC');
	...
	private function createNavigationPermission($tool_id=0) {
		...
		$ar_permission=array();
		$Qselect=osW_Database::getInstance()->query('SELECT * FROM :table_vis_page_permission: WHERE tool_id=:tool_id:');
		$Qselect->bindTable(':table_vis_page_permission:', 'vis_page_permission');
		$Qselect->bindInt(':tool_id:', $tool_id);
		$Qselect->execute();
		if ($Qselect->numberOfRows()>0) {
			while($Qselect->next()) {
	...
	private function checkNavigationPermissionRecursive($navigation_element, $user_id=0, $tool_id=0) {
		...
		if (($navigation_element['info']['page_name_intern']!='')&&(osW_VIS_Permission::getInstance()->checkPermission('link', $navigation_element['info']['page_name_intern'], $user_id, $tool_id)===true)) {
			$navigation_element['info']['permission_link']=true;
		}

		if (($navigation_element['info']['page_name_intern']!='')&&(osW_VIS_Permission::getInstance()->checkPermission('view', $navigation_element['info']['page_name_intern'], $user_id, $tool_id)===true)) {
	...
	public function getPages($tool_id) {
		...
		if (!isset($this->data[$name])) {
			...
			$Qselect=osW_Database::getInstance()->query('SELECT * FROM :table_vis_page: WHERE tool_id=:tool_id: AND page_ispublic=:page_ispublic: ORDER BY page_name ASC, page_name_intern ASC');
			$Qselect->bindTable(':table_vis_page:', 'vis_page');
			$Qselect->bindInt(':tool_id:', $tool_id);
