...
class osW_VIS_Permission extends osW_Object {
	...
	public function loadPermissionText($tool_id=0) {
		...
		if (!isset($this->data[$name])) {
			$Qselect=osW_Database::getInstance()->query('SELECT * FROM :table_vis_permission: WHERE tool_id=:tool_id: AND permission_ispublic=:permission_ispublic:');
	...
	public function loadPermissionsByUserId($user_id=0, $tool_id=0) {
		...
		foreach (osW_VIS_Group::getInstance()->getGroupsByUserId($user_id) as $group_id) {
			$QloadData = osW_Database::getInstance()->query('SELECT * FROM :table_vis_group_permission: AS gp INNER JOIN :table_vis_page: AS p ON (p.page_name_intern=gp.permission_page) INNER JOIN :table_vis_page_permission: AS pp ON (pp.permission_flag=gp.permission_flag AND pp.page_id=p.page_id) WHERE gp.group_id=:group_id:');
			$QloadData->bindTable(':table_vis_group_permission:', 'vis_group_permission');
			$QloadData->bindTable(':table_vis_page:', 'vis_page');
