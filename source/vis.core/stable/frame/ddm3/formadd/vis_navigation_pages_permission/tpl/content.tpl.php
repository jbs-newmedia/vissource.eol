...
<tr class="table_ddm_row table_ddm_row_data <?php echo osW_Template::getInstance()->getColorClass('table_ddm_rows', array('table_ddm_row_cella', 'table_ddm_row_cellb'))?> <?php if(osW_Form::getInstance()->getFormError($element)===true):?>table_ddm_row_error<?php endif?> ddm_element_<?php echo $this->getAddElementValue($ddm_group, $element, 'id')?> ">
	...
	<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
		...
		<?php $permission_list=osW_VIS_Permission::getInstance()->getPermissionTextList(osW_VIS::getInstance()->getToolId())?>
		<?php if(!empty($permission_list)):?>
		<ul class="table_ddm_list table_ddm_list_vertical">
		<?php foreach(osW_VIS_Permission::getInstance()->getPermissionTextList(osW_VIS::getInstance()->getToolId()) as $permission_flag => $permission_text):?>
			<li class="table_ddm_list_element"><?php echo osW_Form::getInstance()->drawCheckBoxField($element.'_'.$permission_flag, '1', ((isset($ar_navigation_permission[$permission_flag])&&($ar_navigation_permission[$permission_flag]==1)) ? 1 : 0), array())?> <?php echo h()->_outputString($permission_text)?></li>
		<?php endforeach?>
		</ul>
		<?php endif?>
	</td>
