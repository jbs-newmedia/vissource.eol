...
$QloadData=osW_Database::getInstance()->query('SELECT * FROM :table_vis_page_permission: WHERE page_id=:page_id: AND tool_id=:tool_id:');
...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	$ar_navigation_permission=array();
	foreach(osW_VIS_Permission::getInstance()->getPermissionTextList(osW_VIS::getInstance()->getToolId()) as $permission_flag => $permission_name) {
