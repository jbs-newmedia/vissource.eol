...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	$ar_user_group=array();
	foreach(osW_VIS_Manager::getInstance()->getGroups(osW_VIS::getInstance()->getToolID()) as $group_id => $group_name_intern) {
		if ((isset($_POST[$element.'_'.$group_id]))&&($_POST[$element.'_'.$group_id]==1)) {
			$ar_user_group[osW_VIS::getInstance()->getToolID()][$group_id]=1;
		} else {
			$ar_user_group[osW_VIS::getInstance()->getToolID()][$group_id]=0;
