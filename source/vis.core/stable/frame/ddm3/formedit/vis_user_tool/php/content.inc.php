...
if ($QloadData->numberOfRows()>0) {
	while ($QloadData->next()>0) {
		$ar_tool_user[$QloadData->Value('tool_id')]=1;
	}
}
...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	$ar_user_tool=array();
	if ((isset($_POST[$element.'_'.osW_VIS::getInstance()->getToolID()]))&&($_POST[$element.'_'.osW_VIS::getInstance()->getToolID()]==1)) {
		$ar_user_tool[osW_VIS::getInstance()->getToolID()]=1;
	} else {
		$ar_user_tool[osW_VIS::getInstance()->getToolID()]=0;
		if (!isset($_POST[$element.'_'.osW_VIS::getInstance()->getToolId()])) {
			$_POST[$element.'_'.osW_VIS::getInstance()->getToolId()]=0;
		}
	}
