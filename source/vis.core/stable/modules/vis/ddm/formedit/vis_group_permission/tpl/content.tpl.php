...
<?php
$ar_permission=$this->getFormDataElement($ddm_group, $element);
				...
				if ((!isset($navigation_page_details['custom']))||($navigation_page_details['custom']!==true)) {
					...
					if (count($navigation_page_details['navigation_permissionflags'])>0) {
						foreach ($navigation_page_details['navigation_permissionflags'] as $flag) {
							echo ' &nbsp; '.osW_Form::getInstance()->drawCheckBoxField($navigation_page_details['navigation_page'].'_'.$flag, '1', (isset($ar_permission[$navigation_page_details['navigation_page']][$flag]) ? $ar_permission[$navigation_page_details['navigation_page']][$flag] : 0), array())?> <?php echo h()->_outputString(osW_VIS_Permission::getInstance()->getPermissionText($flag, $ar_tool['tool_id'])).'<br/>';
						}
					}
							...
							if ((!isset($navigation_subpage_details['custom']))||($navigation_subpage_details['custom']!==true)) {
								...
								if (count($navigation_subpage_details['navigation_permissionflags'])>0) {
									foreach ($navigation_subpage_details['navigation_permissionflags'] as $flag) {
										echo ' &nbsp; &nbsp; &nbsp; '.osW_Form::getInstance()->drawCheckBoxField($navigation_subpage_details['navigation_page'].'_'.$flag, '1', (isset($ar_permission[$navigation_subpage_details['navigation_page']][$flag]) ? $ar_permission[$navigation_subpage_details['navigation_page']][$flag] : 0), array())?> <?php echo h()->_outputString(osW_VIS_Permission::getInstance()->getPermissionText($flag, $ar_tool['tool_id'])).'<br/>';
									}
								}
