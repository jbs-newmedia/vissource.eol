...
$this->data['settings']['data']=array(
	'page_title'=>'Create/Update VIS-Tables',
);
...
if (($position=='run')&&(isset($_POST['next']))&&($_POST['next']=='next')) {
	if ((isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))) {
		...
		$patches=array('osw_vis_badge', 'osw_vis_group', 'osw_vis_mandant', 'osw_vis_navigation', 'osw_vis_permission', 'osw_vis_protect', 'osw_vis_user', 'osw_vis');
		...
		$this->data['messages'][]='VIS-Tables were created/updated successfully';
	} else {
		$this->data['messages'][]='VIS-Tables creation/update was skipped (there is no database configured)';
	}

}

if (($position=='run')&&(isset($_POST['prev']))&&($_POST['prev']=='prev')) {
	$this->data['messages'][]='VIS-Tables creation/update was skipped (go to previous page)';
}
