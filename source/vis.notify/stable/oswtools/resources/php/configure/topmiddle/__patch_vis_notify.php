...
$this->data['settings']['data']=array(
	'page_title'=>'Create/Update VIS-Notify-Tables',
);
...
if (($position=='run')&&(isset($_POST['next']))&&($_POST['next']=='next')) {
	if ((isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))) {
		...
		$this->data['messages'][]='VIS-Notify-Tables were created/updated successfully';
	} else {
		$this->data['messages'][]='VIS-Notify-Tables creation/update was skipped (there is no database configured)';
	}

}

if (($position=='run')&&(isset($_POST['prev']))&&($_POST['prev']=='prev')) {
	$this->data['messages'][]='VIS-Notify-Tables creation/update was skipped (go to previous page)';
}
