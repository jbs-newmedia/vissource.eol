...
class osW_VIS2_Manager extends osW_Object {
	...
	public function getTools() {
		if (!isset($this->data['tools'])) {
			$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis2_tool: WHERE 1 ORDER BY tool_name ASC');
			$QselectTools->bindTable(':table_vis2_tool:', 'vis2_tool');
			$QselectTools->execute();
			if ($QselectTools->query_handler===false) {
				osW_VIS2::getInstance()->__initDB();
				$QselectTools->execute();
			}
	...
	public function getUsers() {
		if (!isset($this->data['users'])) {
			$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis2_user: WHERE 1 ORDER BY user_lastname ASC, user_firstname ASC, user_name ASC');
			$QselectTools->bindTable(':table_vis2_user:', 'vis2_user');
			$QselectTools->execute();
			if ($QselectTools->query_handler===false) {
				osW_VIS2_User::getInstance()->__initDB();
				$QselectTools->execute();
			}
			if ($QselectTools->numberOfRows()>0) {
				while ($QselectTools->next()) {
	...
	public function getGroups($tool_id=0) {
		if (!isset($this->data['groups'])) {
			$QselectTools = osW_Database::getInstance()->query('SELECT * FROM :table_vis2_group: WHERE 1 ORDER BY group_name_intern ASC');
			$QselectTools->bindTable(':table_vis2_group:', 'vis2_group');
			$QselectTools->execute();
			if ($QselectTools->query_handler===false) {
				osW_VIS2_Group::getInstance()->__initDB();
				$QselectTools->execute();
			}
			if ($QselectTools->numberOfRows()>0) {
				while ($QselectTools->next()) {
