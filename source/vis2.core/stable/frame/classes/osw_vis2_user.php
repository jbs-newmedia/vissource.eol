...
class osW_VIS2_User extends osW_Object {
	...
	public function loadUser($user_token) {
		if (strlen($user_token)==32) {
			$QloadData = osW_Database::getInstance()->query('SELECT * FROM :table_vis2_user: WHERE user_token=:user_token:');
			$QloadData->bindTable(':table_vis2_user:', 'vis2_user');
			...
			if ($QloadData->numberOfRows()==1) {
				...
				$Quser=osW_Database::getInstance()->query('SELECT * FROM :table_vis2_user_pref WHERE user_id=:user_id');
				$Quser->bindTable(':table_vis2_user_pref', 'vis2_user_pref');
	...
	private function getData($value) {
		$Quser=osW_Database::getInstance()->query('SELECT * FROM :table_vis2_user_pref WHERE user_id=:user_id AND pref_name=:pref_name');
		$Quser->bindTable(':table_vis2_user_pref', 'vis2_user_pref');
		$Quser->bindInt(':user_id', $this->getId());
	...
	public function doLogin($user_email) {
		...
		$QupdateData = osW_Database::getInstance()->query('UPDATE :table_vis2_user: SET user_token=:user_token: WHERE user_email=:user_email:');
		...
		osW_Session::getInstance()->set('vis2_user_token', $user_token);
		return true;
	}
	...
	public function getIdByEMail($email) {
		if (!isset($this->data['user_id_by_email'][$email])) {
			$QselectUser = osW_Database::getInstance()->query('SELECT user_id FROM :table_vis2_user: WHERE user_email=:user_email:');
			$QselectUser->bindTable(':table_vis2_user:', 'vis2_user');
	...
	public function getDisplayName($full=true) {
		if ($full===true) {
			return trim($this->data['user_form'].' '.$this->data['user_firstname'].' '.$this->data['user_lastname']);
		} else {
			return trim($this->data['user_firstname'].' '.$this->data['user_lastname']);
		}
	}
	...
	public function setUserValue($name, $value, $typ, $userid=0) {
		...
		$time = time();
		$Quser = osW_Database::getInstance()->query('INSERT INTO :table_vis2_user_pref: (user_id, pref_name, pref_type, pref_create, pref_lastupdate, pref_integer, pref_float, pref_string, pref_text, pref_date) VALUE (:user_id:, :pref_name:, :pref_type:, :pref_create:, :pref_lastupdate:, :pref_integer:, :pref_float:, :pref_string:, :pref_text:, :pref_date:) ON DUPLICATE KEY UPDATE pref_lastupdate=:pref_lastupdate:, pref_integer=:pref_integer:, pref_float=:pref_float:, pref_string=:pref_string:, pref_text=:pref_text:, pref_date=:pref_date:');
		$Quser->bindTable(':table_vis2_user_pref:', 'vis2_user_pref');
	...
	public function getUserValue($pref_name, $userid=0) {
		...
		$Quser = osW_Database::getInstance()->query('SELECT * FROM :table_vis2_user_pref: WHERE pref_name=:pref_name: AND user_id=:user_id:');
		$Quser->bindTable(':table_vis2_user_pref:', 'vis2_user_pref');
	...
	public function delUserValue($pref_name, $userid=0) {
		...
		$Quser = osW_Database::getInstance()->query('DELETE FROM :table_vis2_user_pref: WHERE pref_name=:pref_name: AND user_id=:user_id:');
		$Quser->bindTable(':table_vis2_user_pref:', 'vis2_user_pref');
		$Quser->bindValue(':pref_name:', $pref_name);
		$Quser->bindInt(':user_id:', $userid);
		$Quser->execute();
