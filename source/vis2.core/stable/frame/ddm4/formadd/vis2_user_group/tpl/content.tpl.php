...
<div class="form-group ddm_element_<?php echo $this->getAddElementValue($ddm_group, $element, 'id')?>">
	...
	<?php if($this->getAddElementOption($ddm_group, $element, 'read_only')===true):?>
		...
		<?php /* read only */ ?>
		<?php $ar_user_group=$this->getAddElementStorage($ddm_group, $element); ?>
	...
	<?php else:?>
		...
		<?php $i=0;foreach(osW_VIS2_Manager::getInstance()->getGroups(osW_VIS2::getInstance()->getToolID()) as $group_id => $group_name_intern):$i++;?>
			<div class="custom-control custom-checkbox">
				<?php echo osW_Form::getInstance()->drawCheckBoxField($element.'_'.$group_id, '1', ((isset($ar_user_group[osW_VIS2::getInstance()->getToolID()])&&(isset($ar_user_group[osW_VIS2::getInstance()->getToolID()][$group_id]))&&($ar_user_group[osW_VIS2::getInstance()->getToolID()][$group_id]==1)) ? 1 : 0), array('input_parameter'=>'title="'.h()->_outputString($group_name_intern).'"', 'input_class'=>'custom-control-input'))?>
				<label class="custom-control-label<?php if(osW_Form::getInstance()->getFormError($element)):?> text-danger<?php endif?>" for="<?php echo $element.'_'.$group_id ?>0"><?php echo h()->_outputString($group_name_intern)?></label>
			</div>
		<?php endforeach?>
