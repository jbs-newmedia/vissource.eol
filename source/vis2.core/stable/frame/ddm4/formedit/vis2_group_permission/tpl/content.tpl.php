	...
	function vis2_manager_group_permission($ddm_group, $navigation_element, $ar_permission, $ro=false) {
		?>
				...
				<div class="custom-checkbox">
					...
					<?php echo '<i class="fa fa-check" aria-hidden="true"></i> '.h()->_outputString(osW_VIS2_Permission::getInstance()->getPermissionText($flag, osW_VIS2::getInstance()->getToolId()))?>
					<?php else:?>
					<?php echo '<i class="fa fa-times" aria-hidden="true"></i> '.h()->_outputString(osW_VIS2_Permission::getInstance()->getPermissionText($flag, osW_VIS2::getInstance()->getToolId()))?>
					<?php endif?>
				...
				<div class="custom-control custom-checkbox">
					...
					<label class="custom-control-label<?php if(osW_Form::getInstance()->getFormError($element)):?> text-danger<?php endif?>" for="<?php echo 'page_'.$navigation_element['info']['page_name_intern'].'_'.$flag ?>0"><?php echo h()->_outputString(osW_VIS2_Permission::getInstance()->getPermissionText($flag, osW_VIS2::getInstance()->getToolId()))?></label>
				</div>
				<?php endif?>
				<?php endforeach?>
				<?php endif?>
...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($ddm_group, $element, 'id')?>">
	...
	<?php if($this->getEditElementOption($ddm_group, $element, 'read_only')===true):?>
		...
		<?php /* read only */ ?>
		<?php $ar_permission=osW_DDM4::getInstance()->getEditElementStorage($ddm_group, $element)?>
		<?php foreach (osW_VIS2_Navigation::getInstance()->getNavigationReal(0, osW_DDM4::getInstance()->getGroupOption($ddm_group, 'navigation_level'), osW_VIS2::getInstance()->getToolId()) as $navigation_element):?>
	...
	<?php else:?>
		...
		<?php $ar_permission=osW_DDM4::getInstance()->getEditElementStorage($ddm_group, $element)?>
		<?php foreach (osW_VIS2_Navigation::getInstance()->getNavigationReal(0, osW_DDM4::getInstance()->getGroupOption($ddm_group, 'navigation_level'), osW_VIS2::getInstance()->getToolId()) as $navigation_element):?>
		<?php echo vis2_manager_group_permission($ddm_group, $navigation_element, $ar_permission)?>
		<?php endforeach?>
