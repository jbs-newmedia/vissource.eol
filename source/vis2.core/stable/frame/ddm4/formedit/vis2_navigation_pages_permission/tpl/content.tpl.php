...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($ddm_group, $element, 'id')?>">
	...
	<?php if($this->getEditElementOption($ddm_group, $element, 'read_only')===true):?>
		...
		<?php /* read only */ ?>
		<?php $ar_navigation_permission=$this->getEditElementStorage($ddm_group, $element)?>
		<?php $permission_list=osW_VIS2_Permission::getInstance()->getPermissionTextList(osW_VIS2::getInstance()->getToolId())?>
		<?php $c=count(osW_VIS2_Permission::getInstance()->getPermissionTextList(osW_VIS2::getInstance()->getToolId()))?>
		<?php if(!empty($permission_list)):?>
			<?php $i=0;foreach(osW_VIS2_Permission::getInstance()->getPermissionTextList(osW_VIS2::getInstance()->getToolId()) as $permission_flag => $permission_text):$i++;?>
		...
		<?php if(!empty($permission_list)):?>
			<?php $i=0;foreach(osW_VIS2_Permission::getInstance()->getPermissionTextList(osW_VIS2::getInstance()->getToolId()) as $permission_flag => $permission_text):$i++;?>
				<div class="custom-control custom-checkbox">
					<?php echo osW_Form::getInstance()->drawCheckBoxField($element.'_'.$permission_flag, '1', ((isset($ar_navigation_permission[$permission_flag])&&($ar_navigation_permission[$permission_flag]==1)) ? 1 : 0), array('input_parameter'=>'title="'.h()->_outputString($permission_text).'"', 'input_class'=>'custom-control-input'))?>
					<label class="custom-control-label<?php if(osW_Form::getInstance()->getFormError($element)):?> text-danger<?php endif?>" for="<?php echo $element.'_'.$permission_flag ?>0"><?php echo h()->_outputString($permission_text)?></label>
				</div>
			<?php endforeach?>
		<?php endif?>
