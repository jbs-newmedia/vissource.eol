...
class osW_Tool_VIS2 extends osW_Tool_Object {
	...
	public function parseScript1($_vis2_script, $object, $time=0) {
		...
		if ($QreadData->numberOfRows()==0) {
			...
			$QinsertData->bindValue(':tool_status:', $_vis2_script['tool']['tool_status']);
			$QinsertData->bindValue(':tool_hide_logon:', $_vis2_script['tool']['tool_hide_logon']);
			$QinsertData->bindValue(':tool_hide_navigation:', $_vis2_script['tool']['tool_hide_navigation']);
			...
			$QreadData->next();
			$QupdateData=osW_Tool_Database::getInstance()->query('UPDATE :table_vis2_tool: SET tool_name=:tool_name:, tool_description=:tool_description:, tool_status=:tool_status:, tool_hide_logon=:tool_hide_logon:, tool_hide_navigation=:tool_hide_navigation: WHERE tool_id=:tool_id:');
			...
			$QupdateData->bindValue(':tool_status:', $_vis2_script['tool']['tool_status']);
		...
		foreach ($_vis2_script['group'] as $id => $group) {
			...
			$QreadData->execute();
			if ($QreadData->numberOfRows()==0) {
				$QinsertData=osW_Tool_Database::getInstance()->query('INSERT INTO :table_vis2_group: (group_name_intern, group_name, group_description, group_status, tool_id, group_create_time, group_create_user_id, group_update_time, group_update_user_id) VALUES (:group_name_intern:, :group_name:, :group_description:, :group_status:, :tool_id:, :group_create_time:, :group_create_user_id:, :group_update_time:, :group_update_user_id:)');
				...
				$QreadData->next();
				$QupdateData=osW_Tool_Database::getInstance()->query('UPDATE :table_vis2_group: SET group_name=:group_name:, group_description=:group_description:, group_status=:group_status:, tool_id=:tool_id: WHERE group_id=:group_id:');
