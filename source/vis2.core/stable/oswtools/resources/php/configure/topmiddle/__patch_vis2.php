...
$this->data['settings']['data']=array(
	'page_title'=>'Create/Update VIS2-Tables',
);
...
if (($position=='run')&&(isset($_POST['next']))&&($_POST['next']=='next')) {
	if ((isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))) {
		...
		$patches=array('osw_vis2_badge', 'osw_vis2_group', 'osw_vis2_mandant', 'osw_vis2_navigation', 'osw_vis2_permission', 'osw_vis2_protect', 'osw_vis2_user', 'osw_vis2');
		...
		$this->data['messages'][]='VIS2-Tables were created/updated successfully';
	} else {
		$this->data['messages'][]='VIS2-Tables creation/update was skipped (there is no database configured)';
	}

}

if (($position=='run')&&(isset($_POST['prev']))&&($_POST['prev']=='prev')) {
	$this->data['messages'][]='VIS2-Tables creation/update was skipped (go to previous page)';
}
