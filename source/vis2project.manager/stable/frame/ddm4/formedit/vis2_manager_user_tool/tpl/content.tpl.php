...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($ddm_group, $element, 'id')?>">
	...
	<?php else:?>
		...
		<div class="custom-control custom-checkbox">
			<?php echo osW_Form::getInstance()->drawCheckBoxField($element.'_'.$tool_id, '1', ((isset($ar_tool_user[$tool_id])&&($ar_tool_user[$tool_id]==1)) ? 1 : 0), array('input_parameter'=>'title="'.h()->_outputString($tool_name).'"', 'input_class'=>'custom-control-input'))?>
			<label class="custom-control-label<?php if(osW_Form::getInstance()->getFormError($element)):?> text-danger<?php endif?>" for="<?php echo $element.'_'.$tool_id ?>0"><?php echo h()->_outputString($tool_name)?></label>
		</div>
		<?php endforeach?>
