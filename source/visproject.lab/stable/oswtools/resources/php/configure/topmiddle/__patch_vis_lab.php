...
$this->data['settings']['data']=array(
	'page_title'=>'Create/Update VIS-Lab-Tables',
);
...
if (($position=='run')&&(isset($_POST['next']))&&($_POST['next']=='next')) {
	if ((isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))&&(isset($this->data['values_json']['database_db']))) {
		...
		if (defined('root_path')) {
			$dir=root_path.'frame/classes_patch/osw_vis_lab/';
		...
		$this->data['messages'][]='VIS-Lab-Tables were created/updated successfully';
	} else {
		$this->data['messages'][]='VIS-Lab-Tables creation/update was skipped (there is no database configured)';
	}
}

if (($position=='run')&&(isset($_POST['prev']))&&($_POST['prev']=='prev')) {
	$this->data['messages'][]='VIS-Lab-Tables creation/update was skipped (go to previous page)';
}
