	...
	<td class="table_ddm_col table_ddm_col_data table_ddm_col_form">
		...
		<?php foreach(osW_VIS_Manager::getInstance()->getTools() as $tool_id => $tool_name):?>
			...
			<?php foreach(osW_VIS_Manager::getInstance()->getGroups($tool_id) as $group_id => $group_name_intern):?>
				<li class="table_ddm_list_element"><?php echo osW_Form::getInstance()->drawCheckBoxField($element.'_'.$tool_id.'_'.$group_id, '1', ((isset($ar_user_group[$tool_id])&&(isset($ar_user_group[$tool_id][$group_id]))&&($ar_user_group[$tool_id][$group_id]==1)) ? 1 : 0), array())?> <?php echo h()->_outputString($group_name_intern)?></li>
			<?php endforeach?>
			</ul>
			</li>
		<?php endforeach?>
