...
$ar_user_group=array();
$QloadData=osW_Database::getInstance()->query('SELECT * FROM :table_vis_user_group: WHERE user_id=:user_id:');
...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	$ar_user_group=array();
	foreach(osW_VIS_Manager::getInstance()->getTools() as $tool_id => $tool_name) {
		foreach(osW_VIS_Manager::getInstance()->getGroups($tool_id) as $group_id => $group_name_intern) {
