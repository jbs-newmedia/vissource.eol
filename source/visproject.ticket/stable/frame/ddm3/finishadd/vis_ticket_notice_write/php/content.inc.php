...
for($i=1;$i<=10;$i++) {
	if ((isset($notice_data['notice_data'.$i.'_md5']))&&($notice_data['notice_data'.$i.'_md5']!='')) {
...
osW_VIS_Ticket::getInstance()->setTicketNotification($ticket_data['ticket_id'], $notice_data['notice_create_user']);
...
if ($adresses!=array()) {
	...
	foreach ($adresses as $key => $emails) {
		foreach ($emails as $email) {
			switch ($key) {
				case 'user':
					$mail['info']='Bei einem dir zugewiesen Ticket wurde eine Notiz erstellt.';
					break;
				case 'group':
					$mail['info']='Bei einem deiner Gruppe zugewiesen Ticket wurde eine Notiz erstellt.';
					break;
				case 'default':
					$mail['info']='Bei einem Ticket ohne Zuordnung wurde eine Notiz erstellt.';
					break;
				case 'notify':
				default:
					$mail['info']='Bei einem Ticket wurde eine Notiz erstellt.';
					break;
			}
