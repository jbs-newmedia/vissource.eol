...
foreach ($this->getDeleteElements($ddm_group) as $element => $element_details) {
	if ((isset($element_details['name']))&&($element_details['name']!='')) {
		$notice_data_old[$element]=$this->getDeleteElementStorage($ddm_group, $element);
	}
}

$notice_data_old['notice_create_time']=$this->getDeleteElementStorage($ddm_group, 'notice_create_time');
$notice_data_old['notice_create_user_id']=$this->getDeleteElementStorage($ddm_group, 'notice_create_user_id');
$notice_data_old['notice_update_time']=$this->getDeleteElementStorage($ddm_group, 'notice_update_time');
$notice_data_old['notice_update_user_id']=$this->getDeleteElementStorage($ddm_group, 'notice_update_user_id');

for ($i=1;$i<=10;$i++) {
	$notice_data_old['notice_data'.$i.'_name']=$this->getDeleteElementStorage($ddm_group, 'notice_data'.$i.'_name');
	$notice_data_old['notice_data'.$i.'_type']=$this->getDeleteElementStorage($ddm_group, 'notice_data'.$i.'_type');
	$notice_data_old['notice_data'.$i.'_size']=$this->getDeleteElementStorage($ddm_group, 'notice_data'.$i.'_size');
	$notice_data_old['notice_data'.$i.'_md5']=$this->getDeleteElementStorage($ddm_group, 'notice_data'.$i.'_md5');
}
...
if (isset($ticket_users[$notice_data_old['notice_create_user_id']])) {
	$notice_data_old['notice_create_user']=$ticket_usersfl[$notice_data_old['notice_create_user_id']];
} else {
	$notice_data_old['notice_create_user']='';
}
if (isset($ticket_users[$notice_data_old['notice_update_user_id']])) {
	$notice_data_old['notice_update_user']=$ticket_usersfl[$notice_data_old['notice_update_user_id']];
} else {
...
for($i=1;$i<=10;$i++) {
	if ((isset($notice_data_old['notice_data'.$i.'_md5']))&&($notice_data_old['notice_data'.$i.'_md5']!='')) {
...
$mail['notice']['notice_description']['current']=$notice_data_old['notice_description'];
$mail['notice']['notice_description']['old']='';
$mail['notice']['notice_description']['changed']=false;

$mail['notice']['notice_data']=array();
$mail['notice']['notice_data']['info']='Dateien';
$mail['notice']['notice_data']['current']=$notice_data_old['notice_data'];
$mail['notice']['notice_data']['old']='';
$mail['notice']['notice_data']['changed']=false;

$mail['notice']['notice_create_time']=array();
$mail['notice']['notice_create_time']['info']='Erstellt';
$mail['notice']['notice_create_time']['current']=date('d.m.Y, H:i', $notice_data_old['notice_create_time']).' Uhr von '.$notice_data_old['notice_create_user'];
$mail['notice']['notice_create_time']['old']='';
...
$mail['notice']['notice_update_time']['current']=date('d.m.Y, H:i', $notice_data_old['notice_update_time']).' Uhr von '.$notice_data_old['notice_update_user'];
$mail['notice']['notice_update_time']['old']='';
...
if ($adresses!=array()) {
	...
	foreach ($adresses as $key => $emails) {
		foreach ($emails as $email) {
			switch ($key) {
				case 'user':
					$mail['info']='Bei einem dir zugewiesen Ticket wurde eine Notiz gelöscht.';
					break;
				case 'group':
					$mail['info']='Bei einem deiner Gruppe zugewiesen Ticket wurde eine Notiz gelöscht.';
					break;
				case 'default':
					$mail['info']='Bei einem Ticket ohne Zuordnung wurde eine Notiz gelöscht.';
					break;
				case 'notify':
				default:
					$mail['info']='Bei einem Ticket wurde eine Notiz gelöscht.';
					break;
			}
