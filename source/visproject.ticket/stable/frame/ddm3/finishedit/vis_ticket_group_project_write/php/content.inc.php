...
foreach ($ar_tool_user_do as $tool_id => $flag) {
	if ((!isset($ar_tool_user[$tool_id]))||($ar_tool_user[$tool_id]!==$flag)) {
		if ($flag==1) {
			$QinsertData=osW_Database::getInstance()->query('INSERT INTO :table_vis_ticket_group_project: (group_id, project_id, group_project_create_time, group_project_create_user_id, group_project_update_time, group_project_update_user_id) VALUES (:group_id:, :project_id:, :group_project_create_time:, :group_project_create_user_id:, :group_project_update_time:, :group_project_update_user_id:)');
			$QinsertData->bindTable(':table_vis_ticket_group_project:', 'vis_ticket_group_project');
			...
			$QdeleteData=osW_Database::getInstance()->query('DELETE FROM :table_vis_ticket_group_project: WHERE group_id=:group_id: AND project_id=:project_id:');
			$QdeleteData->bindTable(':table_vis_ticket_group_project:', 'vis_ticket_group_project');
