...
foreach ($ar_tool_user_do as $tool_id => $flag) {
	if ((!isset($ar_tool_user[$tool_id]))||($ar_tool_user[$tool_id]!==$flag)) {
		if ($flag==1) {
			$QinsertData=osW_Database::getInstance()->query('INSERT INTO :table_vis_ticket_user_pemgroup: (group_id, user_id, user_group_create_time, user_group_create_user_id, user_group_update_time, user_group_update_user_id) VALUES (:group_id:, :user_id:, :user_group_create_time:, :user_group_create_user_id:, :user_group_update_time:, :user_group_update_user_id:)');
			$QinsertData->bindTable(':table_vis_ticket_user_pemgroup:', 'vis_ticket_user_pemgroup');
			$QinsertData->bindInt(':group_id:', $tool_id);
			...
			$QdeleteData=osW_Database::getInstance()->query('DELETE FROM :table_vis_ticket_user_pemgroup: WHERE group_id=:group_id: AND user_id=:user_id:');
			$QdeleteData->bindTable(':table_vis_ticket_user_pemgroup:', 'vis_ticket_user_pemgroup');
			$QdeleteData->bindInt(':group_id:', $tool_id);
