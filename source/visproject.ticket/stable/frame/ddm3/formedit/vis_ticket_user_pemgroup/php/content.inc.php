...
$ar_tool_user=array();
$QloadData=osW_Database::getInstance()->query('SELECT * FROM :table_vis_ticket_user_pemgroup: WHERE user_id=:user_id:');
$QloadData->bindTable(':table_vis_ticket_user_pemgroup:', 'vis_ticket_user_pemgroup');
$QloadData->bindInt(':user_id:', $this->getIndexElementStorage($ddm_group));
$QloadData->execute();
if ($QloadData->numberOfRows()>0) {
	while ($QloadData->next()>0) {
		$ar_tool_user[$QloadData->Value('group_id')]=1;
	}
}
...
if (osW_Settings::getInstance()->getAction()=='doedit') {
	$ar_user_tool=array();
	foreach(osW_VIS_Ticket::getInstance()->getPemGroups2Users() as $tool_id => $tool_name) {
