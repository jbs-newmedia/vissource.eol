...
foreach (osW_VIS_Ticket::getInstance()->getAllUsers() as $user_id => $user_name) {
	...
	$QgetData = osW_Database::getInstance()->query('SELECT * FROM :table_vis_ticket: WHERE user_id=:user_id: AND ticket_enddate=:ticket_enddate: AND project_id IN (:project_id:) AND status_id IN (:status_id:) ORDER BY ticket_enddate ASC, importance_id DESC, ticket_number DESC');
	$QgetData->bindTable(':table_vis_ticket:', 'vis_ticket');
	$QgetData->bindInt(':user_id:', $user_id);
	$QgetData->bindInt(':ticket_enddate:', 0);
	...
	$QgetData->bindTable(':table_vis_ticket:', 'vis_ticket');
	$QgetData->bindInt(':user_id:', $user_id);
	$QgetData->bindInt(':ticket_enddate:', date('Ymd'), mktime(date('h'), date('i'), date('s'), date('m'), date('d')+14, date('Y')));
	$QgetData->bindRaw(':project_id:', implode(',', $ar_projects));
	$QgetData->bindRaw(':status_id:', implode(',', 	osW_VIS_Ticket::getInstance()->getStatusData(true, false, false, false, false, false, false, 'status_internal_id', 'status_internal_id')+osW_VIS_Ticket::getInstance()->getStatusData(false, false, true, false, false, false, false, 'status_internal_id', 'status_internal_id')+osW_VIS_Ticket::getInstance()->getStatusData(false, false, false, true, false, false, false, 'status_internal_id', 'status_internal_id')));
	$QgetData->execute();
	if ($QgetData->numberOfRows()>0) {
		while ($QgetData->next()) {
			$mail['future'][]=$QgetData->result;
		}
	}

	if (($mail['null']!=array())||($mail['future']!=array())) {
		$mail['info']='Hier ist eine Übersicht deiner Tickets, die in Planung sind oder noch kein Ablaufdatum haben.';
		...
		osW_VIS_Mailer::getInstance()->setSubject('Übersicht deiner geplanten Tickets');
