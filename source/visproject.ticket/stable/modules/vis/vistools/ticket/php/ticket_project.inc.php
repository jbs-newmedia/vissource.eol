...
$project_data=osW_VIS_Ticket::getInstance()->getProjectsByUserId();

if (!isset($project_data[osW_Session::getInstance()->value('ticket_project_id')])) {
	h()->_direct(osW_Template::getInstance()->buildhrefLink(vOut('frame_default_module'), 'vistool='.osW_VIS::getInstance()->getTool()));
}

$page=osW_VIS_Navigation::getInstance()->getPage();

osW_VIS_Navigation::getInstance()->setPage($page.'_'.osW_Session::getInstance()->value('ticket_project_id'));
...
foreach(osW_VIS_Ticket::getInstance()->getProjectsByUserId(0, 'project_id', 'array') as $project_details) {
	if ($project_details['project_parent_id']>0) {
		$_project_ids[$project_details['project_parent_id']][]=$project_details['project_id'];
	} else {
		$_project_ids[$project_details['project_id']][]=$project_details['project_id'];
	}
}
...
if (($ticket_data!=array())&&($view=='ticket')) {
	...
	if (isset($_project_ids[osW_Session::getInstance()->value('ticket_project_id')])) {
		$project_ids=$_project_ids[osW_Session::getInstance()->value('ticket_project_id')];
	} else {
		$project_ids=array();
	}
	...
	$ar_project_ids=array_flip($project_ids);

	foreach ($ar_projects as $project_id => $project_details) {
		if (!isset($ar_project_ids[$project_id])) {
			unset($ar_projects[$project_id]);
		}
	}
	...
	$getcount=osW_VIS_Ticket::getInstance()->getCounter('project_id IN ('.implode(',', $project_ids).')');
	...
	if (in_array($ddm_navigation_id, array(1))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'ticket_create_time ', 'operator'=>'>', 'value'=>(time()-(60*60*24*7)))))), 'database');
	...
	if (in_array($ddm_navigation_id, array(2))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'ticket_update_time ', 'operator'=>'>', 'value'=>(time()-(60*60*24*7)))))), 'database');
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'order', array('ticket_number'=>'desc'), 'database');
	}

	if (in_array($ddm_navigation_id, array(3))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'status_id ', 'operator'=>'IN', 'value'=>'('.implode(',', (osW_VIS_Ticket::getInstance()->getStatusDataOpen(false, 'status_internal_id', 'status_internal_id'))).')')))), 'database');
	}

	if (in_array($ddm_navigation_id, array(6))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'status_id ', 'operator'=>'IN', 'value'=>'('.implode(',', (osW_VIS_Ticket::getInstance()->getStatusDataWaiting(false, 'status_internal_id', 'status_internal_id'))).')')))), 'database');
	}

	if (in_array($ddm_navigation_id, array(4))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'status_id ', 'operator'=>'IN', 'value'=>'('.implode(',', (osW_VIS_Ticket::getInstance()->getStatusDataAssigned(false, 'status_internal_id', 'status_internal_id'))).')')))), 'database');
	}

	if (in_array($ddm_navigation_id, array(7))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'status_id ', 'operator'=>'IN', 'value'=>'('.implode(',', (osW_VIS_Ticket::getInstance()->getStatusDataInWork(false, 'status_internal_id', 'status_internal_id'))).')')))), 'database');
	}

	if (in_array($ddm_navigation_id, array(5))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')'), array('key'=>'status_id ', 'operator'=>'IN', 'value'=>'('.implode(',', (osW_VIS_Ticket::getInstance()->getStatusDataClosed(false, 'status_internal_id', 'status_internal_id'))).')')))), 'database');
	}

	if (in_array($ddm_navigation_id, array(99))) {
		osW_DDM3::getInstance()->setGroupOption($ddm_group, 'filter', array(array('and'=>array(array('key'=>'project_id ', 'operator'=>'IN', 'value'=>'('.implode(',', $project_ids).')')))), 'database');
	}
...
osW_VIS_Navigation::getInstance()->setPage($page);
