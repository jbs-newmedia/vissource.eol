...
if ($dbstatus->error===false) {
	...
	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_tool: WHERE 1');
	$Qget->bindTable(':table_vis2_tool:', 'vis2_tool');
	$Qget->execute();
	$dbclean2delete['vis2_tool']=array();
	...
	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_mandant: WHERE 1');
	$Qget->bindTable(':table_vis2_mandant:', 'vis2_mandant');
	$Qget->execute();
	$dbclean2delete['vis2_mandant']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if (in_array($Qget->result['tool_id'], $dbclean['vis_tool_array'])) {
				...
				$dbclean2delete['vis2_mandant'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_permission: WHERE 1');
	$Qget->bindTable(':table_vis2_permission:', 'vis2_permission');
	$Qget->execute();
	$dbclean2delete['vis2_permission']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if (in_array($Qget->result['tool_id'], $dbclean['vis_tool_array'])) {
				...
				$dbclean2delete['vis2_permission'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_page: WHERE 1');
	$Qget->bindTable(':table_vis2_page:', 'vis2_page');
	$Qget->execute();
	$dbclean2delete['vis2_page']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if (in_array($Qget->result['tool_id'], $dbclean['vis_tool_array'])) {
				...
				$dbclean2delete['vis2_page'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_page_permission: WHERE 1');
	$Qget->bindTable(':table_vis2_page_permission:', 'vis2_page_permission');
	$Qget->execute();
	$dbclean2delete['vis2_page_permission']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if ((in_array($Qget->result['tool_id'], $dbclean['vis_tool_array']))&&(in_array($Qget->result['page_id'], $dbclean['vis_page_array'][$Qget->result['tool_id']]))&&(in_array($Qget->result['permission_flag'], $dbclean['vis_permission_array'][$Qget->result['tool_id']]))) {
				...
				$dbclean2delete['vis2_page_permission'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_navigation: WHERE 1 ORDER BY navigation_parent_id ASC');
	$Qget->bindTable(':table_vis2_navigation:', 'vis2_navigation');
	$Qget->execute();
	$dbclean2delete['vis2_navigation']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if ((in_array($Qget->result['tool_id'], $dbclean['vis_tool_array']))&&(($Qget->result['navigation_parent_id']==0)||(in_array($Qget->result['navigation_parent_id'], $dbclean['vis_navigation_array'][$Qget->result['tool_id']])))) {
				...
				$dbclean2delete['vis2_navigation'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_group: WHERE 1');
	$Qget->bindTable(':table_vis2_group:', 'vis2_group');
	$Qget->execute();
	$dbclean2delete['vis2_group']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if (in_array($Qget->result['tool_id'], $dbclean['vis_tool_array'])) {
				...
				$dbclean2delete['vis2_group'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_group_permission: AS gp LEFT JOIN :table_vis2_group: AS g ON (gp.group_id=g.group_id) LEFT JOIN :table_vis2_page: AS p ON (p.page_name_intern=gp.permission_page AND p.tool_id=g.tool_id) WHERE 1');
	$Qget->bindTable(':table_vis2_group_permission:', 'vis2_group_permission');
	$Qget->bindTable(':table_vis2_group:', 'vis2_group');
	$Qget->bindTable(':table_vis2_page:', 'vis2_page');
	$Qget->execute();
	...
	$dbclean2delete['vis2_group_permission']=array();
	...
	foreach ($vis_group_cache as $vis_group_permission) {
		$dbclean2delete['vis2_group_permission'][]=$vis_group_permission;
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_user: WHERE 1');
	$Qget->bindTable(':table_vis2_user:', 'vis2_user');
	$Qget->execute();
	$dbclean2delete['vis2_user']=array();
	...
	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_user_pref: WHERE 1');
	$Qget->bindTable(':table_vis2_user_pref:', 'vis2_user_pref');
	$Qget->execute();
	$dbclean2delete['vis2_user_pref']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if (in_array($Qget->result['user_id'], $dbclean['vis_user_array'])) {
				...
				$dbclean2delete['vis2_user_pref'][]=$Qget->result;
			}
		}
	}

	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_user_tool: WHERE 1');
	$Qget->bindTable(':table_vis2_user_tool:', 'vis2_user_tool');
	$Qget->execute();
	$dbclean2delete['vis2_user_tool']=array();
	...
	if ($Qget->numberOfRows()>0) {
		while ($Qget->next()) {
			if ((in_array($Qget->result['tool_id'], $dbclean['vis_tool_array']))&&(in_array($Qget->result['user_id'], $dbclean['vis_user_array']))) {
				...
				$dbclean2delete['vis2_user_tool'][]=$Qget->result;
			}
		}
	}
	...
	$Qget = osW_Tool_Database::getInstance()->query('SELECT * FROM :table_vis2_user_group: WHERE 1');
	$Qget->bindTable(':table_vis2_user_group:', 'vis2_user_group');
	$Qget->execute();
	...
	$dbclean2delete['vis2_user_group']=array();
	...
	foreach ($vis_user_group_cache as $vis_user_group) {
		$dbclean2delete['vis2_user_group'][]=$vis_user_group;
	}
		...
		foreach ($dbclean2delete as $key => $entries) {
			if (count($entries)>0) {
				foreach ($entries as $entry) {
					osW_Tool_VIS_DB2Clear::getInstance()->clearEntry($key, $entry);
				}
			}
		}
	}
...
echo osW_Tool_Template::getInstance()->outputHeader(array('../resources/css/layout_core.css', '../resources/css/vis.db2clear.css'), array('../resources/js/jquery.js'));
